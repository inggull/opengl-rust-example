...
impl Context {
    pub fn create() -> Result<Context, errors::Error> {
        ...
        unsafe {
            ...
            gl::GenBuffers(2, vbo.as_mut_ptr());  // 새로운 vbo를 생성
            spdlog::info!("Created vertex buffer({})", vbo[0]);
            spdlog::info!("Created vertex buffer({})", vbo[1]);

            // 초기에 `VertexAttribPointer`의 포인터는 각 버택스 버퍼에 바인딩된 배열의 시작 주소를 가리켰지만, vao가 등장하면서 vao가 가리키는 배열의 오프셋을 의미하게 되었다
            // attribute #0: position
            gl::BindBuffer(gl::ARRAY_BUFFER, vbo[0]);  // 사용할 vbo를 지정
            gl::BufferData(gl::ARRAY_BUFFER, (size_of::<f32>() * 18).cast_signed(), vertices.as_ptr().cast(), gl::STATIC_DRAW);
            gl::EnableVertexAttribArray(0);
            gl::VertexAttribPointer(0, 3, gl::FLOAT, gl::FALSE, (size_of::<f32>() * 6) as i32, (size_of::<f32>() * 0) as *const _);

            // attribute #1: color
            gl::BindBuffer(gl::ARRAY_BUFFER, vbo[1]);  // 사용할 vbo를 지정
            gl::BufferData(gl::ARRAY_BUFFER, (size_of::<f32>() * 18).cast_signed(), vertices.as_ptr().cast(), gl::STATIC_DRAW);
            gl::EnableVertexAttribArray(1);
            gl::VertexAttribPointer(1, 3, gl::FLOAT, gl::FALSE, (size_of::<f32>() * 6) as i32, (size_of::<f32>() * 3) as *const _);
            ...
            gl::BufferData(gl::ELEMENT_ARRAY_BUFFER, (size_of::<i32>() * 3).cast_signed(), indices.as_ptr().cast(), gl::STATIC_DRAW);
        }
...
impl Drop for Context {
    fn drop(&mut self) {
        unsafe {
            ...
            gl::DeleteBuffers(2, self.vbo.as_mut_ptr());
            gl::DeleteBuffers(1, &self.ebo);
        }
        spdlog::info!("Dropped vertex array({})", self.vao);
        spdlog::info!("Dropped vertex buffer({})", self.vbo[0]);
        spdlog::info!("Dropped vertex buffer({})", self.vbo[1]);
